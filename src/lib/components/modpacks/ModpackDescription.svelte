<script lang="ts">
  ...
  $: modsList = $modResultsStore.map((r) => r?.data?.mod ?? null).filter((m): m is NonNullable<typeof m> => Boolean(m));
        ...
        <svelte:fragment slot="panel">
          ...
          {:else if tabSet === 'list'}
            <div class="flex items-center justify-end gap-4">
              <button
                class="material-symbols-outlined text-4xl transition hover:text-orange-400"
                on:click={() => (cardView = true)}>grid_view</button>
              <button
                class="material-symbols-outlined text-4xl transition hover:text-orange-400"
                on:click={() => (cardView = false)}>view_list</button>
            </div>
            {#if cardView}
              <div class="mb-5 ml-auto mt-5 flex justify-end">
                <div>
                  <Paginator
              ...
              <div class="grid grid grid-cols-1 gap-4 gap-6 lg:grid-cols-2">
                {#each mods as mod}
                  <ModCard {mod} />
                {/each}
              </div>
            ...
            {:else}
              <div class="max-h-[70vh] w-full overflow-y-auto">
                <table class="mt-4 w-full text-sm">
                  <thead class="sticky top-0 z-10 border-b border-white/10 bg-zinc-900 backdrop-blur">
                      ...
                      <tr class="hover:bg-white/5">
                        ...
                        <td class="px-4 py-3">
                          <div class="flex justify-end gap-3">
                            <button
                              class="variant-ghost-primary btn min-w-[140px]"
                              on:click={() => goto(`/mod/${mod.id}`)}>{$t('modpack.list-view.view-mod')}</button>
                            <button
                              class="variant-ghost-primary btn min-w-[160px]"
                              on:click={() => goto(`/mod/${mod.id}/version/${mod.latestVersions.release.id}`)}
                              >{$t('modpack.list-view.patch-notes')}</button>
                          </div>
                        </td>
                      </tr>
                    {/each}
