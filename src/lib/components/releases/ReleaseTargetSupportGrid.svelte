<script lang="ts">
  import type { ModpackTarget } from '$lib/generated';
  import { getTranslate } from '@tolgee/svelte';
  import { T } from '@tolgee/svelte';

  export const { t } = getTranslate();

  export let targets: Pick<ModpackTarget, 'target_name'>[] = [];
  ...
  $: HasWindows = targets.some((target) => target.target_name === 'Windows');
  $: HasWindowsServer = targets.some((target) => target.target_name === 'WindowsServer');
  $: HasLinuxServer = targets.some((target) => target.target_name === 'LinuxServer');
  $: WindowsSupport = {
